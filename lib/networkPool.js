...
function NetworkPool() {
  ...
  scanner.get().on('networks', function (foundNetworks) {
    ...
    for (let i = 0; i < self.networks.length; i++) {
    ...
    for (let i = 0; i < foundNetworks.length; i++) {
      let network = _.find(self.networks, {extendedPanId: foundNetworks[i].extendedPanId});
      if (!network) {
        // new one!
...
NetworkPool.prototype.reset = function () {
  logger.info('Resetting Network Pool!');
  this.networks      = [];
  this.surveyNetwork = {};
};
...
let networkPool = new NetworkPool();
