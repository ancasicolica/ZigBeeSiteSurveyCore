...
module.exports = function (options) {
  let x;
  let lowerX  = options.frequency - options.bandwith / 2;
  let upperX  = options.frequency + options.bandwith / 2;
  let samples = options.samples || 10;
  ...
  let system = gaussianElimination.solve(matrix, result);
  ...
  for (x = lowerX; x < upperX; x += (upperX - lowerX) / samples) {
    let y = Math.abs(system.solution[0].toNumber() * Math.pow(x, 2) + system.solution[1].toNumber() * x + system.solution[2].toNumber());
